...
|原则||
|-|-|
|单一职责|就一个类、接口、方法而言，应该仅有一个引起它变化的原因
开放封闭|软件的实体（类、模块、函数），对于扩展开放、对于修改关闭
依赖倒转|抽象不应该依赖于细节，细节应该依赖于抽象；高层也不依赖底层，而共同依赖抽象（接口）
里氏替换|把父类对象用子类对象代替，程序的行为没有变。
合成聚合复用|尽量使用合成/聚合，尽量不使用继承
...
## 内联函数
1. 可以在函数声明前加上`inline`语句，进行显性定义或者声明。
2. 小的成员函数如果在类内定义，一般编译器会默认为内联函数。
3. 复杂语句（循环、多分支）不适合内联。
4. 内联函数的定义和声明不能分开。
...
从复用的结构而言，对于有继承和组合的类，先执行包含的类或继承的父类的构造函数，以文本上的定义顺序执行构造，而与构造函数中`ClassA():ClassB(x),ClassC(y){}`的执行顺序无关。
...
```
构造函数不允许滞后调用，在定义了以后调用。故可以引入新语法，对类中嵌套的类初始化（拷贝构造函数），调用那个类的构造函数：
```cpp
class Student
{
    string name;
    StudentID id;
public:
    Student(string pName,int ssID=0):id(ssID)   //新语法
    {
        name=pName;
    }
}; 
```
对于引用成员和常量成员，不能用构造函数对它们赋值，所以这被认为是没有意义的。
## 关于动态内存
...
这样的话，就可以采用**虚继承**，在继承时加上`virtual`关键字。这样的话，`M_P`则是分别继承自`Master`、`PhD`（这两个也继承自`Student`）和`Student`三个基类。但是，这个时候`Student* p=&M_P`是成立的，因为，`Student*`指针是明确的，就是`M_P`继承的那个`Student`基类。`Master`、`PhD`的`Student`基类会产生一个虚指针，供这两个类在访问它们的父类时使用。虚拟继承的基类直接派生类构造函数放弃向上传导。
